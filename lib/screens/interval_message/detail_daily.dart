...
class DailyMessageDetailScreen extends StatefulWidget {
  ...
  const DailyMessageDetailScreen({super.key, required this.message});

  @override
  DailyMessageDetailScreenState createState() =>
      DailyMessageDetailScreenState();
}

class DailyMessageDetailScreenState extends State<DailyMessageDetailScreen> {
        ...
        child: Column(
          ...
          children: [
            ...
            if (widget.message.createdAt != null)
              _buildMetadataRow(
                  '생성일', DateFormatter.formatDateTime24(widget.message.createdAt!)),
            if (widget.message.updatedAt != null)
              _buildMetadataRow(
                  '수정일', DateFormatter.formatDateTime24(widget.message.updatedAt!)),
  ...
  void _saveMessage() async {
    ...
    final success = await controller.updateDailyMessage(
      widget.message.id!,
      updatedMessage,
    );
        ...
        actions: [
          ...
          TextButton(
            onPressed: () async {
              ...
              if (widget.message.id != null) {
                final success =
                await controller.deleteDailyMessage(widget.message.id!);
                if (success) {
                  if (mounted) {
